...
const BottomSheetContent = ({id, startTimesArray, minParticipants, maxParticipants, price}) => {
  const navigate = useNavigate();
  const [selectedDate, setSelectedDate] = useState(null);
  const [selectedTime, setSelectedTime] = useState(null);
  const [selectedPeople, setSelectedPeople] = useState(null);

  const timeOptions = startTimesArray?.map(item => item.startTime) || [];
  const peopleOptions = Array.from({ length: maxParticipants - minParticipants + 1 }, (_, idx) => minParticipants + idx);
